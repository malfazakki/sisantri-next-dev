...
export function DivisionView() {
	...
	const filteredDivisions = divisions?.filter((division) =>
		...
		division.id.toLowerCase().includes(search.toLowerCase())
	);

	// Pagination logic
	const totalPages = Math.ceil((filteredDivisions?.length || 0) / rowsPerPage);
	const paginatedDivisions = useMemo(() => {
		const start = (page - 1) * rowsPerPage;
		const end = start + rowsPerPage;
		return filteredDivisions?.slice(start, end);
	}, [filteredDivisions, page, rowsPerPage]);
			...
			<div className='flex items-start justify-between'>
				<div>
					...
					<p className='mt-2 text-sm font-medium text-slate-500'>
						Here's a list of all divisions in your organization
					</p>
				</div>
											...
											<td colSpan={3} className='px-6 py-16 text-center'>
												<div className='flex flex-col items-center justify-center'>
													...
													<p className='text-sm font-medium text-slate-700 mb-1'>
														{search ? 'No results found' : 'No divisions yet'}
													</p>
													<p className='text-xs text-slate-500'>
														{search
															? 'Try adjusting your search criteria'
															: 'Get started by creating a new division'}
													</p>
												</div>
											</td>
										</tr>
