...
import { Plus, Loader2, Trash2, Search, MoreHorizontal, Edit, SlidersHorizontal, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Shield } from "lucide-react";
...
export function RoleView() {
	...
	// Pagination logic
	const totalPages = Math.ceil((filteredRoles?.length || 0) / rowsPerPage);
											...
											<td colSpan={4} className='px-6 py-16 text-center'>
												<div className='flex flex-col items-center justify-center'>
													...
													<p className='text-sm font-medium text-slate-700 mb-1'>
														{search ? 'No results found' : 'No roles yet'}
													</p>
										...
										paginatedRoles?.map((role) => (
											...
											>
												<td className='px-6 py-4'>
													<span className={`inline-flex px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wider border ${getRoleBadgeStyle(role.name)}`}>
														{role.name}
													</span>
												</td>
												<td className='px-6 py-4'>
